...
Because every array in a ZVF store is an ordinary Zarr array and the
...
A **fragment** is ZVF's addressing primitive: one entry in a chunk's
...
ids — ZVF stores an additional `object_index` group whose
...
read coordinate transforms from a ZVF store without modification. The block
...
at each level. The ZVF discriminator is
`multiscales[0].metadata.format == "zarr_vectors"`.

ZVF's own fields are kept out of that block rather than mixed into it, so a
...
ZVF supports seven geometry types, each identified by a string constant in
